...
public class TPAAutoTasks implements PIDOutput{
	...
	public void startTask(){
		if((Math.abs(TPARobotDrive.frontLeftMotor.getPosition()) < 4180) &&
				(taskCounterA == 0)){
		...
		if(taskCounterA == 1){
			...
			if(Robot.gearSwitch.get() == true){
				Robot.leftServo.set(0);
				Robot.rightServo.set(.74);
				stopAndReset2();
			}else{
				makeStraight(.5);
			}
		...
		if((Math.abs(TPARobotDrive.frontLeftMotor.getPosition()) < 750) &&
				(taskCounterA == 2)){
		...
		if((Math.abs(TPARobotDrive.frontLeftMotor.getPosition()) < 1350) &&
				(taskCounterA == 3)){
			//Robot.robotDrive.arcadeDrive(-0.35, 0.0);
			makeStraight(-0.6);
		}
